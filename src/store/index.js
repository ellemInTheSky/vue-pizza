...
export default createStore({
  ...
  getters: {
    ...
    getTotalPrice(state) {
      return state.cart.reduce((a, b) => a + b.price * b.counter, 0);
    },
  },
  mutations: {
    ...
    filterPizzas(state, category) {
      state.filteredPizzas = state.pizzas.filter(item => item.category === category);
    },
    sortPizzas (state, sort) {
      state.filteredPizzas = state.filteredPizzas.slice().sort( (a,b) => a[sort] > b[sort] ? 1: -1);
    }
  },
  actions: {},
  modules: {},
